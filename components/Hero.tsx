    ...
    <div className="relative pt-20 pb-32 flex flex-col items-center justify-center text-center px-4 overflow-hidden">
      ...
      <div className="relative z-10 max-w-5xl">
        <div className="inline-flex items-center gap-2 mb-6 bg-primary/10 border border-primary/30 rounded-full px-4 py-1 animate-bounce">
          ...
          <span className="text-[10px] text-primary tracking-[0.3em] font-bold uppercase">System Online: v4.0_Stable</span>
        </div>
        ...
        <h1 className="text-6xl md:text-8xl font-bold tracking-tighter leading-none mb-8 hover:animate-glitch transition-all cursor-default select-none">
          Architecting <span className="text-primary hologram-glow">Digital</span> <br />
          <span className="text-primary hologram-glow">Solutions</span>: Full Stack, <br />
          WordPress, & <span className="text-accent-purple purple-glow">Deployment</span>
        </h1>
        ...
        <p className="text-slate-400 text-lg md:text-xl max-w-3xl mx-auto mb-12 font-light tracking-wide animate-pulse [animation-duration:4s]">
          ...
          Specializing in high-performance web architecture and scalable cloud deployment.
        </p>
        
        <div className="flex flex-col md:flex-row items-center justify-center gap-6">
          <a href="#projects" className="w-full md:w-auto px-10 py-4 bg-primary text-background-dark font-bold uppercase tracking-widest hover:bg-white transition-all rounded-sm flex items-center justify-center gap-2 group shadow-[0_0_20px_rgba(37,209,244,0.4)]">
            Explore My Work <span className="material-icons group-hover:translate-x-1 transition-transform">arrow_forward</span>
          </a>
          <button className="w-full md:w-auto px-10 py-4 border border-primary/40 text-slate-100 font-bold uppercase tracking-widest hover:bg-primary/10 transition-all rounded-sm group relative overflow-hidden">
      ...
      <div className="mt-24 grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-16 max-w-5xl w-full border-t border-primary/10 pt-12">
        {[
          ...
          { id: '02', label: 'NODE / PYTHON' },
          { id: '03', label: 'AWS / DOCKER' },
          { id: '04', label: 'WORDPRESS HEADLESS' },
        ].map((item, i) => (
          <div key={item.id} className="text-left group cursor-default hover:translate-y-[-5px] transition-transform duration-300" style={{ animationDelay: `${i * 0.2}s` }}>
            <span className="text-primary font-bold text-xl block mb-1 group-hover:hologram-glow transition-all">{item.id}</span>
            <span className="text-[10px] text-primary/60 font-mono tracking-widest uppercase group-hover:text-primary transition-colors">{item.label}</span>
          </div>
        ))}
      </div>
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-4 opacity-40">
        ...
        <span className="text-[8px] font-mono tracking-[0.4em] uppercase">Scroll to Explore</span>
      </div>
    </div>
  );
};
