    ...
    <section id="projects" className="py-24 overflow-hidden">
      ...
      <div className="w-full overflow-x-auto snap-x snap-mandatory flex gap-8 px-8 md:px-[calc(50vw-640px+32px)] no-scrollbar py-12">
        {PROJECTS.map((project) => (
          <div
            key={project.id}
            className="flex-shrink-0 w-[300px] md:w-[600px] aspect-[4/5] md:aspect-[16/10] snap-center group relative"
          >
            <div className="relative w-full h-full glass-panel rounded-xl overflow-hidden group-hover:scale-[1.02] transition-all duration-500 border-primary/20">
              ...
              <div className="absolute top-0 left-0 w-full p-6 flex justify-between items-start z-20">
                <div>
                  <span className="bg-primary/20 text-primary px-3 py-1 text-[10px] font-mono tracking-widest rounded-full border border-primary/30">
                    ID: {project.id}
                  </span>
                  <h3 className="text-2xl font-bold mt-2 tracking-tight uppercase">
                    {project.title}
                  </h3>
                </div>
              ...
              <div className="absolute inset-0 p-8 flex flex-col justify-end z-30 translate-y-20 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 bg-gradient-to-t from-background-dark via-background-dark/90 to-transparent">
                ...
                <div className="flex flex-wrap gap-2 mb-8">
                  {project.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-3 py-1 bg-primary/10 text-primary text-[10px] font-mono border border-primary/20 rounded uppercase"
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <div className="flex gap-4">
                  {project.demoUrl && (
                    <a
                      ...
                      className="flex-1 bg-primary text-background-dark font-bold py-3 rounded-lg flex items-center justify-center gap-2 hover:bg-white transition-colors group/btn uppercase tracking-widest text-xs"
                    >
                      Initialize{" "}
                  ...
                  {project.repoUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-14 bg-background-dark/50 border border-primary text-primary py-3 rounded-lg flex items-center justify-center hover:bg-primary/10 transition-colors"
                      title="View Code"
                    >
